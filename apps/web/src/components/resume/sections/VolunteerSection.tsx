...
export function VolunteerSection({ profileId, onDataChange }: { profileId: string; onDataChange?: () => void }) {
  const { entries, loading, saving, error, addEntry, updateEntry, deleteEntry } =
    useSectionData<VolunteerEntry>(profileId, 'volunteer', onDataChange);
  const [editing, setEditing] = useState<string | null>(null);
  const [adding, setAdding] = useState(false);
  const [form, setForm] = useState<Omit<VolunteerEntry, 'id'>>({
    organization: '',
    role: '',
    startDate: '',
    endDate: '',
  ...
  const resetForm = () => {
    setForm({ organization: '', role: '', startDate: '', endDate: '', description: '' });
    setAdding(false);
    setEditing(null);
  };

  const startEdit = (entry: VolunteerEntry) => {
  ...
  const handleSave = async () => {
    if (editing) {
      ...
      await addEntry(form as VolunteerEntry);
    }
    resetForm();
  };
    ...
    <div className="space-y-3">
      ...
      {adding ? (
        ...
        <Button size="sm" variant="outline" className="w-full" onClick={() => { resetForm(); setAdding(true); }}>
          <Plus className="h-3.5 w-3.5" /> Add Volunteer Experience
        </Button>
      )}
    </div>
  );
