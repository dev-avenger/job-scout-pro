...
export function CertificationsSection({ profileId, onDataChange }: { profileId: string; onDataChange?: () => void }) {
  const { entries, loading, saving, error, addEntry, updateEntry, deleteEntry } =
    useSectionData<CertificationEntry>(profileId, 'certifications', onDataChange);
  const [editing, setEditing] = useState<string | null>(null);
  const [adding, setAdding] = useState(false);
  const [form, setForm] = useState<Omit<CertificationEntry, 'id'>>({
    name: '',
    issuer: '',
    date: '',
    expiryDate: '',
  ...
  const resetForm = () => {
    setForm({ name: '', issuer: '', date: '', expiryDate: '', credentialUrl: '' });
    setAdding(false);
    setEditing(null);
  };

  const startEdit = (entry: CertificationEntry) => {
  ...
  const handleSave = async () => {
    if (editing) {
      ...
      await addEntry(form as CertificationEntry);
    }
    resetForm();
  };
  ...
  const renderForm = (isEditing: boolean) => (
    <div className={`space-y-3 rounded-lg border ${isEditing ? 'bg-muted/20' : 'border-dashed bg-muted/10'} p-4`}>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <Input placeholder="Credential URL (optional)" value={form.credentialUrl} onChange={(e) => setForm({ ...form, credentialUrl: e.target.value })} className="sm:col-span-2" />
      </div>
        ...
        editing === entry.id ? (
          ...
          <div key={entry.id} className="group flex items-start justify-between rounded-lg border bg-muted/20 p-4">
            <div className="min-w-0 flex-1">
              ...
              {entry.credentialUrl && (
                <a href={entry.credentialUrl} target="_blank" rel="noreferrer" className="text-xs text-blue-500 hover:underline">
                  View Credential
                </a>
              )}
            </div>
